...
f_DBLocationStrings = open('dictionary_database_test.txt','r')
...
#                             print(f'DEBUG: TwtLatitude is {TwtLongitude}')
        ...
        with open('data_mmda_traffic_alerts - Copy.csv','r',newline='') as CsvFile:
            ...
            #print('Checkpoint 1: opening CSV file')
            # Get last element of the CSV for duplicate check
            
            #     for line in CsvFile:
            if len(ListDuplicateCheck) == 0:
            ...
            if TwtId not in ListDuplicateCheck:
                ...
                if HeaderCheck != []:
                # Then there is data already, just append
                    #print(f'\nCSV HEADER CHECK PASS')
                    ...
                    print(f'\nCSV HEADER CHECK FAILED! Adding header to CSV file')
